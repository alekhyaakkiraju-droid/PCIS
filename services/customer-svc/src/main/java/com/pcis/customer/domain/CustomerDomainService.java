...
public class CustomerDomainService {
  ...
  public CustomerDomainService(
      CustomerRepository customerRepository, CustomerOutboxWriter customerOutboxWriter) {
  ...
  private CustomerEntity persistNewCustomer(CreateCustomerCommand command) {
    ...
    if (command.addresses() != null) {
      command.addresses().forEach(a -> customer.addAddress(toAddressEntity(a)));
    }
    if (command.contacts() != null) {
      command.contacts().forEach(c -> customer.addContact(toContactEntity(c)));
    }
  ...
  public CustomerEntity update(UpdateCustomerCommand command) {
    CustomerEntity customer =
        customerRepository
            .findById(command.custId())
            .orElseThrow(() -> new CustomerNotFoundException(command.custId()));
    assertTaxIdAvailable(command.taxId(), command.custId());
    if (StringUtils.hasText(command.taxId())) {
      customer.setTaxId(normalizeTaxId(command.taxId()));
    }
    if (StringUtils.hasText(command.custName())) {
      customer.setCustName(command.custName());
    }
    if (StringUtils.hasText(command.custType())) {
      customer.setCustType(command.custType());
    }
    if (StringUtils.hasText(command.custStatus())) {
      customer.setCustStatus(command.custStatus());
    }
    CustomerEntity saved = customerRepository.save(customer);
  ...
  private void writeOutboxEvent(CustomerEntity customer, String eventType) {
    ...
    UUID idempotencyKey =
        UUID.nameUUIDFromBytes(
            (AGGREGATE_TYPE + ":" + customer.getCustId() + ":" + eventType).getBytes());
    customerOutboxWriter.writeDomainEvent(
        String.valueOf(customer.getCustId()), eventType, payload, idempotencyKey);
  }
}
